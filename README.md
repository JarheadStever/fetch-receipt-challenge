...
## Thoughts
***A note on Validation:*** I think that ultimately, the way I abstracted the validation a bit into a more applicative instead of monadic style is a good approach and would be beneficial for testing/logging. (If a receipt is invalid, the API will return 404, but an in-depth error message will [log on the command line](https://github.com/JarheadStever/fetch-receipt-challenge/blob/main/handlers.go#L39).) That said, it also made things a bit more complex than I think was necessary for this exercise. The simple validation ([see the original code here](https://github.com/JarheadStever/fetch-receipt-challenge/blob/simple-validation/receipt.go#L26-L79)) also met the requirements and I was torn on which route to go for my submission. Interesting to think about...
